...
bool MAVLink_routing::check_and_forward(mavlink_channel_t in_channel, const mavlink_message_t* msg)
{
    ...
    if (msg->msgid == MAVLINK_MSG_ID_POSITIONING_DATA) {
        // positioning_data needs special handling
        handle_positioning_data(in_channel, msg);
        return true;
    }
...
void MAVLink_routing::learn_route(mavlink_channel_t in_channel, const mavlink_message_t* msg)
{
    ...
    for (i=0; i<num_routes; i++) {
        if (routes[i].sysid == msg->sysid && 
            ...
            } else if (routes[i].mavtype == 227 && msg->msgid == MAVLINK_MSG_ID_POSITIONING_DATA) {
                routes[i].mavtype = mavlink_msg_positioning_data_get_type(msg);
            }
            break;
        }
    }
    if (i == num_routes && i<MAVLINK_MAX_ROUTES) {
        ...
        if (msg->msgid == MAVLINK_MSG_ID_HEARTBEAT) {
            routes[i].mavtype = mavlink_msg_heartbeat_get_type(msg);
        } else if (msg->msgid == MAVLINK_MSG_ID_POSITIONING_DATA) {
            routes[i].mavtype = mavlink_msg_positioning_data_get_type(msg);
        }
        num_routes++;
#if ROUTING_DEBUG
...
*/
void MAVLink_routing::handle_positioning_data(mavlink_channel_t in_channel, const mavlink_message_t* msg)
{
    uint16_t mask = GCS_MAVLINK::active_channel_mask();
