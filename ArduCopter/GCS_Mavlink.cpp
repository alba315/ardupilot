...
void Copter::gcs_send_positioning_data(void)
{
    gcs_send_message(MSG_POSITIONING_DATA);
}
...
NOINLINE void Copter::send_positioning_data(mavlink_channel_t chan)
{
    
    uint8_t base_mode = MAV_MODE_FLAG_CUSTOM_MODE_ENABLED;
    char drone_ID[24] = {'1','0','4','5','0','0','2','F','3','1','3','4','5','1','0','4','3','2','3','6','3','8','3','4'};
    uint32_t custom_mode = control_mode;
    //pulseID declarado como variable global
    pulse_ID = pulse_ID + 1;

    /***** PASAR DE STRING A ARRAY DE CHAR *******/
    //std::string id = "0045002F3134510432363834";
    //char* drone_ID = new char [id.size()+1];
    //strcpy(drone_ID, id.c_str());
    /*********************************************/
    ...
    mavlink_msg_positioning_data_send(
        chan,
#if (FRAME_CONFIG == QUAD_FRAME)
        MAV_TYPE_QUADROTOR,
#elif (FRAME_CONFIG == TRI_FRAME)
...
bool GCS_MAVLINK::try_send_message(enum ap_message id)
{
    ...
    /*|*|*|*|*|AÑADIDO|*|*|*|*|*/
    case MSG_POSITIONING_DATA:    // ID:227
        CHECK_PAYLOAD_SIZE(POSITIONING_DATA);
        copter.gcs[chan-MAVLINK_COMM_0].last_heartbeat_time = hal.scheduler->millis();
        copter.send_positioning_data(chan);
        break;
    /*|*|*|*|*|AÑADIDO|*|*|*|*/
...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_POSITIONING_DATA:      // MAV ID: 227
    {
        // We keep track of the last time we received a positioning_data from our GCS for failsafe purposes
        if(msg->sysid != copter.g.sysid_my_gcs) break;
        copter.failsafe.last_heartbeat_ms = hal.scheduler->millis();
        copter.pmTest1++;
        break;
